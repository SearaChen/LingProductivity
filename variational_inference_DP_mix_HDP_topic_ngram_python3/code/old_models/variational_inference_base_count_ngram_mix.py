...
class VariationalInference(object):
	def __init__(
		...
		self.n=n
		logger.info('The base distribution is %i-gram with base count.' % (n))
		...
		dirichlebase_count_counts = dirichlet_concentration * np.ones(num_symbols)
		...
		self.dirichlet_ngram = DirichletNgram(n, dirichlebase_count_counts, full_contexts, num_sublex, self.customers, self)
		
		self.dirichlet_ngram.set_varpar_assignment()
		self._update_word_likelihood()
		
		logger.info('# of tables: %i' % base_count)
		logger.info('Gamma priors on concent_priorsration: (%f,%f)'
							% (concent_priors[0],concent_priors[1]**-1)
							)
		logger.info('Base count of Dirichlet: %s' % str(dirichlebase_count_counts))
...
# 				encoding='utf-8'
			...
			for context_length,level in self.dirichlet_ngram.tree.items():
				vpc = self.dirichlet_ngram.varpar_concents[context_length]
...
# 						encoding="utf-8"
			...
			df_atom = pd.DataFrame(
						self.dirichlet_ngram.tree[0][()].varpar_atom.flatten()[:,np.newaxis],
						columns=['dirichlet_par']
						)
			num_sublex,num_clusters,num_symbols = self.dirichlet_ngram.tree[0][()].varpar_atom.shape
...
# 							encoding='utf-8'
	...
	def update_varpars(self):
		...
		self.dirichlet_ngram.update_varpars()
		self._update_word_likelihood()
	...
	def get_var_bound(self):
		...
		return (
				...
				+
				self.dirichlet_ngram.get_var_bound()
				+
...
class DirichletNgram(object):
	def __init__(self, num_clusters, n, concent_priors, dirichlebase_count_counts, full_contexts, num_sublex, customers, wrapper):
...
# 								]
	...
	n = int(sys.argv[3])
	base_count = len(decoder)*2 # Number of symbols x 2
