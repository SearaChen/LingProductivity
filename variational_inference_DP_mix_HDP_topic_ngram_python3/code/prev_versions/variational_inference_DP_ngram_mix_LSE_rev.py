...
class VariationalInference(object):
	def __init__(
		...
		self.logger.info('Noise in inheritance of symbols from ngram tables to customers: %f' % noise)
# 		print np.exp(error_mat).sum(axis=1)
		...
		self.T = T
		self.logger.info('(max) # of tables for sublexicalization: %i' % T)
		
		
		self.concent = concent
		self.logger.info('Concentration for sublexicalization: %f' % concent)
		
		self.hdp_ngrams = [HDPNgram(T_base, n, base_concent, table_id, self.customers, self)
							for table_id in range(T)]
		
		
		self.logger.info('(max) # of tables for ngram: %i' % T_base)
		self.logger.info('Concentration for ngram: %f' % base_concent)
	...
	def train(self, max_iters, min_increase):
		...
		self.logger.info("Will be terminated if variational bound is only improved by <=%f." % min_increase)
		converged=False
		iter_id=0
...
# 		self.intermediate_var_bound=self.current_var_bound
		while iter_id<max_iters:
			...
			self.logger.info("Current var_bound is %0.12f (%+0.12f)." % (new_var_bound,improvement))
# 			print improvement
			if np.isnan(new_var_bound):
# 				self.save_results(decoder,inventory)
				raise Exception("nan detected.")
			if improvement<0:
				self.logger.error("variational bound decreased. Something wrong.")
# 				self.save_results(decoder,inventory)
		...
		self.logger.info('Final variational bound is %f.' % self.current_var_bound)
		

		
	def save_results(self, decoder):
		inventory=list(decoder.values())
...
# 		np.savetxt(self.result_path+"sublexicalization_assignment.csv", self.varpar_assignment, delimiter=",")
		...
		for hdp in self.hdp_ngrams:
			for context,bottom_rst in hdp.iter_bottom_restaurants():
				...
				df_assignment.to_csv(self.result_path+("ngram-bottom_%s_assignment.csv" % decoded_context),
										index=False,
										encoding='utf-8'
										)
				...
				df_stick.to_csv(self.result_path+("ngram-bottom_%s_stick.csv" % decoded_context),
								index=False,
								encoding='utf-8'
								)
				...
				df_label.to_csv(self.result_path+("ngram-bottom_%s_label.csv" % decoded_context),
								index=False,
								encoding='utf-8'
								)
...
# 		return self.hdp_ngrams.get_var_bound()
		...
		return (
				np.sum(
					self.digamma_diffs[:,1]*(self.concent-1)
					) # E[log p(V | alpha)]
				+np.sum(
					[
				...
				+(
					np.sum(self.digamma_diffs[:,1]*np.sum(cumsum_phis[:,1:], axis=0))
					+
...
class HDPNgram(object):
	def __init__(self, T, n, concent, id, customers, wrapper):
		self.id=id
		...
		for context_length in range(1,n-1):
...
class DP(object):
	...
	def get_var_bound(self):
		return self.get_sum_E_log_p_varpars()-self.get_E_log_varpar()
	...
	def get_sum_E_log_p_varpars(self):
		...
		return (
					...
					+
					np.sum(
							self.digamma_diffs[:,1]*np.sum(cumsum_phis[...,1:], axis=(0,1))
							+
...
class DP_bottom(object):
	...
	def get_sum_E_log_p_varpars(self):
		...
		return (
					...
					+
					np.sum(
							self.digamma_diffs[:,1]*np.sum(cumsum_phis[...,1:], axis=0)
							+
...
class DP_top(object):
	...
	def get_sum_E_log_p_varpars(self):
		...
		return (
					...
					+ # E[log p(eta_m | Z_m-1, eta_m-1)] is a constant.
					np.sum(
							self.digamma_diffs[:,1]*np.sum(cumsum_phis[...,1:], axis=(0,1))
							+
							self.digamma_diffs[:,0]*np.sum(self.varpar_assignment[...,:-1], axis=(0,1))
...
# 								]
	...
	base_concent = np.float64(sys.argv[5])
	noise = np.float64(sys.argv[6])
	max_iters = int(sys.argv[7])
	min_increase = np.float64(sys.argv[8])
