...
class VariationalInference(object):
	def __init__(
		...
		self.hdp_ngrams = [HDPNgram(base_num_clusters, n, concent_priors, table_id, self.customers, self)
							for table_id in range(num_sublex)]
		
		
		logger.info('(max) # of tables for ngram: %i' % base_num_clusters)
...
# 							encoding='utf-8'
			...
			for sblx_id,hdp in enumerate(self.hdp_ngrams):
				df_concent = pd.DataFrame(hdp.varpar_concent_shape[:,np.newaxis],
											columns=['shape']
											)
				df_concent['scale']=hdp.varpar_concent_inv_scale**-1
				df_concent['context_length']=list(hdp.tree.keys())
				hdf5_store.put(
								"sublex/sublex_%i/concentration" % sblx_id,
								df_concent,
	# 							encoding='utf-8'
								)
				for context_length,level in hdp.tree.items():
					for context,rst in level.items():
						...
						if context_length==0:
							...
							atom_shape = rst.varpar_atom.shape
							...
							hdf5_store.put(
											("sublex/sublex_%i/_1gram/context_/atom" % sblx_id)
											,
											df_atom
											)
						...
						hdf5_store.put(
							("sublex/sublex_%i/_%igram/context_%s/assignment"
							% (sblx_id,context_length+1,coded_context))
							,
							df_assignment
							,
	# 						encoding='utf-8'
						...
						hdf5_store.put(
							("sublex/sublex_%i/_%igram/context_%s/stick"
							% (sblx_id,context_length+1,coded_context))
							,
							df_stick
							,
...
class HDPNgram(object):
	def __init__(self, num_clusters, n, concent_priors, id, customers, wrapper):
		self.id=id
		self.wrapper=wrapper
		self.n = n
		self.tree = {k:{} for k in range(n)}
		self.concent_priors=concent_priors
		self.varpar_concent_inv_scale = np.random.gamma(
													1,#concent_priors[1]**-1,
													20,
													size=n
													) # The INVERSE scale of a gamma distr.
		...
		[ngram.enter_a_restaurant(self.tree[n-1][ngram.context])
...
# 		
	...
	def get_log_posterior_pred_per_word(self, test_word):
		...
		for ngram in test_word.ngrams:
			for minus_conlen,context_length in enumerate(reversed(range(self.n))):
				sub_context=ngram.context[minus_conlen:]
				if sub_context in iter(self.tree[context_length].keys()):
...
class DP_bottom(object):
	...
	def set_varpar_assignment(self):
		...
		self.log_like = (self.varpar_assignment
						*
						self.hdp.get_word_assignment_weights(self.word_ids)[:,np.newaxis] # num_clusters x num_symbols
						)
	...
	def get_var_bound(self):
		self.word_assignment_weights = self.hdp.get_word_assignment_weights(self.word_ids) # num_customers
...
# 								]
	...
	n = int(sys.argv[3])
	base_num_clusters = len(decoder)*2 # Number of symbols x 2
