...
def list_probable_words(
	...
	df.head(n=list_length).to_csv(os.path.join(result_dir, 'top-%i-probable_%s-words_with-respect-to-sublex-%i.csv' % (list_length, true_sublex, sublex_id)), index=False, encoding='utf-8')
	df.tail(n=list_length).to_csv(os.path.join(result_dir, 'worst-%i-probable_%s-words_with-respect-to-sublex-%i.csv' % (list_length, true_sublex, sublex_id)), index=False, encoding='utf-8')
...
if __name__ == '__main__':
	...
	kanji2alph=dict([('漢', 'SJ'), ('和','Native'), ('混', 'Mixed'), ('外', 'Foreign'), ('固', 'Proper'), ('記号', 'Symbols')])
