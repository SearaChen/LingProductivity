...
class VariationalInference(object):
	def __init__(
		...
		num_1_mora_or_less = np.sum(np.array(list(segment2mora.values()))<=1)
	...
	def save_results(self, decoder):
		...
		with pd.HDFStore(
			...
			for context_length,level in self.hdp_ngram.last_tree.items():
...
# 						encoding="utf-8"
			...
			for context_length,level in self.hdp_ngram.non_last_tree.items():
				vpc = self.hdp_ngram.varpar_concents[context_length]
				df_concent_sub = pd.DataFrame(
									vpc.rate[:,np.newaxis]
									,
...
class HDPNgram(object):
	def __init__(
		...
		self.last_tree = {k:{} for k in range(n)}
		self.non_last_tree = {k:{} for k in range(n)}
	...
	def set_varpar_assignment(self):
		[restaurant.set_varpar_assignment()
			for level in self.last_tree.values()
				for restaurant in level.values()
				]
		[restaurant.set_varpar_assignment()
			for level in self.non_last_tree.values()
				for restaurant in level.values()
				]
	...
	def get_var_bound(self):
		return (
				np.sum([restaurant.get_var_bound()
					for level in reversed(list(self.non_last_tree.values()))
						for restaurant in level.values()
						])
				+
				np.sum([restaurant.get_var_bound()
					for level in reversed(list(self.last_tree.values()))
						for restaurant in level.values()
						])
				+
...
def code_data(training_data,test_data=None):
	...
	for symbol in inventory:
		if symbol in ['äː','iː','ɯː','eː','oː']:
			long_vowels.append(symbol)
		else:
			code = len(encoder)
			decoder[code] = symbol
			encoder[symbol] = code
			if 'ː' in symbol or symbol in ['ä','i','ɯ','e','o']:
				segment2mora[code] = 1
			else:
				segment2mora[code] = 0
