...
# 							encoding='utf-8'
	...
	# 									)
						hdf5_store.put(
							("sublex/sublex_%i/_%igram/context_%s/label"
							% (sblx_id,context_length+1,coded_context))
							,
							df_label
							,
...
# 		self.concent_log_mean=sps.digamma(self.varpar_concent[0])-np.log(self.varpar_concent[1])
	...
	def set_varpar_assignment(self):
		...
		self.log_error_mat_obs = log_error_mat[self.customers]
		assert self.varpar_assignment.size, 'Empty restaurant created.'
		self.phi_x_tau_x_error = np.matmul(
								self.varpar_assignment,
								np.matmul(
...
def code_data(training_data,test_data=None):
	...
	decoder[0]='START' # Special code reserved for initial symbol.
	decoder[1]='END' # Special code reserved for end symbol.
...
# 								]
	...
	result_path = os.path.join(
						'/om/user/tmorita/results_indep_base/',
						# './results',
