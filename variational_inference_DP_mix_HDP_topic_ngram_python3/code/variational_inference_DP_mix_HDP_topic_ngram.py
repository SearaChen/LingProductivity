...
class VariationalInference(object):
	def __init__(
		...
		if inventory_size is None:
			num_symbols = len(set(reduce(lambda x,y: x+y, data)))+1
		else:
		...
		self.data = [Word(word, n, id) for id,word in enumerate(data)]
		data_size = len(data)
		logger.info('# of words: %i' % data_size)
...
# 							encoding='utf-8'
	...
	def get_var_bound(self):
		"""
		Calculate the variational lower bound (i.e. evidential lower bound) based on the current variational parameters.
	...
	def get_sum_E_log_p_varpars(self):
		return (
				...
				+
				np.sum(
					self.word_E_log_likelihood # num_words x num_sublex
					*
					self.varpar_assignment
					) # E[log p(X | Z,eta)]
				) # E[log p(V, alpha)]
	...
	def get_var_constant(self):
		return (
				self.varpar_concent.get_var_constant()
				+
				self.hdp_ngram.get_var_constant()
				+
				self.get_E_log_p_constant()
				)

	def get_E_log_p_constant(self):
		return 0.0 # gammaln(1) = 0



		
class HDPNgram(object):
	def __init__(self, num_clusters, n, concent_priors, dirichlet_base_counts, full_contexts, num_sublex, data, wrapper):
	...
	def get_var_constant(self):
		return (np.sum([restaurant.get_var_constant()
					for level in reversed(list(self.tree.values()))
						for restaurant in level.values()
						])
				+
				np.sum([vpc.get_var_constant() for vpc in self.varpar_concents])
				)
...
class VarParConcent(object):
	...
	def get_var_constant(self):
		return self.prior_shape*np.log(self.prior_rate) - sps.gammaln(self.prior_shape)
...
class VarParConcent_sublex(object):
	...
	def get_var_constant(self):
		return self.rate.size*(self.prior_shape*np.log(self.prior_rate) - sps.gammaln(self.prior_shape))
...
class DP(DP_bottom):
	...
	def get_var_bound(self):
		self._update_log_like_bottom_up()
		return self.get_sum_E_log_p_varpars()-self.get_E_log_q_varpars()
...
class DP_top(DP):
	...
	def get_var_constant(self):
		return sps.gammaln(np.sum(self.atom_base_counts)) - np.sum(sps.gammaln(self.atom_base_counts)) # gammaln(1) = 0
...
# 								]
	coded_data,encoder,decoder = code_data(df.IPA_csv)
