...
def get_entropy(df, tokens={'ʨ,i,r,o':1, 'k,e,n,i':1, 's,ɯ,j,ä':1}):
	rows = []
...
def get_surprisal(df, tokens={'ʨ,i,r,o':1, 'k,e,n,i':1, 's,ɯ,j,ä':1}):
	rows = []
	groups = ['c_type','v_length','trial_name','prefix']
	for (c_type,v_length,trial_name,prefix),sub_df in df.groupby(groups):
		print(sub_df)
...
if __name__ == '__main__':
	...
	df['prefix'] = pd.Categorical(df.prefix, categories=['ʨ,i,r,o', 'k,e,n,i', 's,ɯ,j,ä'], ordered=True)
