...
def kanji2alph(class_name):
	in_eng = class_name.replace('漢', 'SJ').replace('和','Native').replace('混', 'Mixed').replace('外', 'Foreign').replace('固', 'Proper').replace('記号', 'Symbols').replace(':','-')
...
if __name__ == '__main__':
	...
	df = df[df.pos.str.startswith('名') & df.features.map(is_rendaku_applicable)]
