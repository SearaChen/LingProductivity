...
def list_representative_words(
	...
	df.head(n=list_length).to_csv(os.path.join(result_dir, 'top-%i-representative_%s-words_with-respect-to-sublex-%i.csv' % (list_length, true_sublex, sublex_id)), index=False, encoding='utf-8')
	df.tail(n=list_length).to_csv(os.path.join(result_dir, 'worst-%i-representative_%s-words_with-respect-to-sublex-%i.csv' % (list_length, true_sublex, sublex_id)), index=False, encoding='utf-8')
