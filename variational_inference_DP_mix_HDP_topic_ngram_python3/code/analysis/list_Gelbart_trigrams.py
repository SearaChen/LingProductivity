...
penultimate_in_prefix = ['n','j','ɾ']
last_in_prefix = ['i','a','o']
consonants = ['pːː','p','b','bːː','d','dːː']
vowels = ['a','aː']
...
def barplot_log_probs(df_ngram, result_dir):
	...
	for (prefix,c_type), sub_df in df_ngram.groupby(['prefix','c_type']):
		...
		plt.title('p(%s(ːː)a(ː)END | %s)' % (c_type,prefix))
		plt.ylim(ymin=ymin)
...
if __name__ == '__main__':
	...
	df_ngram.loc[:,'decoded_value'] = df_ngram.decoded_value.str.replace('ä','a')
	df_context = df_ngram.decoded_context.str.replace('ä','a').str.replace('r','ɾ').str.split('_', expand=True).rename(columns={0:'c1', 1:'c2'})
	...
	df_ngram_v.loc[:,'penult_in_prefix'] = df_ngram_v.last_in_prefix.map(dict(list(zip(last_in_prefix,penultimate_in_prefix))))
	df_ngram_END = df_ngram[(df_ngram.c1.isin(consonants)) & (df_ngram.c2.isin(vowels)) & (df_ngram.decoded_value == 'END')].rename(columns={'c1':'target_c','c2':'target_v','decoded_value':'END','prob':'END_prob'})
