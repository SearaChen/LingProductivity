...
# 		print np.exp(error_mat).sum(axis=1)
		...
		self.log_varpar_assignment = np.random.dirichlet(np.ones(T), num_customers) # phi in Blei and Jordan.
		...
		self.base_distrs = [HDPNgram(T_base, n, base_concent, table_id, self) for table_id in range(T)]
		for bd in self.base_distrs:
			[ngram.enter_a_restaurant(bd, n)
				for word in self.customers
				for ngram in word.ngrams
				]
			bd.set_log_varpar_assignment()
	...
	def save_results(self):
		np.savetxt(self.result_path+"sublexicalization_log_stick.csv", self.varpar_stick, delimiter=",")
		np.savetxt(self.result_path+"sublexicalization_log_assignment.csv", self.log_varpar_assignment, delimiter=",")
		self.logger.info("Variational parameters saved.")
		
	def _update_varpar_stick(self):
		varpar_assignment=np.exp(self.log_varpar_assignment)
		self.varpar_stick[...,0] = np.sum(varpar_assignment, axis=0)[:-1]+1
...
# 		print self.varpar_stick
	...
	def _update_log_varpar_assignment(self):
...
# 										)
		...
		self.log_varpar_assignment=log_varpar_assignment-spm.logsumexp(log_varpar_assignment, axis=-1)[:,np.newaxis]
# 		print self.log_varpar_assignment
	...
	def update_varpars(self):
		...
		assert not np.any(np.isnan(self.log_varpar_assignment)), ('assignment\n',self.log_varpar_assignment)
		new_KL=self.get_KL_bound()
...
# 					+
# 					np.sum(digamma_diffs[:,0]*np.sum(self.log_varpar_assignment[:,:-1], axis=0))
# 					) # E[log p(Z | V)]
# 		print -(
...
# 					) # E[log q(V)]
# 		print -np.sum(
# 					self.log_varpar_assignment*np.exp(self.log_varpar_assignment)
# 					) # E[log q(Z)]
# 		print np.sum(
# 					np.array(
# 						[
...
# 						table.get_E_log_likelihoods() # E[log p(X | Z, eta)]
				...
				+(
					np.sum(digamma_diffs[:,1]*np.sum(cumsum_phis[:,1:], axis=0))
					+
					np.sum(digamma_diffs[:,0]*np.sum(varpar_assignment[:,:-1], axis=0))
					) # E[log p(Z | V)]
				-(
					np.sum(digamma_diffs[:,0]*(self.varpar_stick[:,0]-1))
					+
...
# 						axis=1
				...
				-np.sum(
					self.log_varpar_assignment*np.exp(varpar_assignment)
					) # E[log q(Z)]
				+np.sum(
					np.array(
						[
...
class HDPNgram(object):
	def __init__(self, T, n, concent, id, whole_dp):
		...
		for context_length in range(1,n):
			for context in itertools.product(list(range(num_symbols)),repeat=context_length):
				for initial_fill_length in range(n-context_length):
	...
	def get_word_assignment_weights(self, word_ids):
		return self.whole_dp.log_varpar_assignment[word_ids,self.id]
	...
	def set_log_varpar_assignment(self):
		[restaurant.set_log_varpar_assignment()
			for level in self.tree.values()
				for restaurant in level.values()
				]
...
class DP(object):
	def __init__(self, T, concent, mother):
		...
		self.varpar_stick = np.random.gamma(10, 0.1, size=(T-1,2)) # gamma in Blei and Jordan.
# 								for i in xrange(n)]
		self.log_varpar_label = np.log(np.random.dirichlet(np.ones(num_symbols), T))
...
# 		self.children_label = [child.log_varpar_label for child in self.children]
		...
		self.num_tables_child = self.children[0].log_varpar_label.shape[0] # T for children.
		self.log_varpar_assignment = np.log(np.random.dirichlet(
											np.ones(self.T),
...
# 								)
		self.phi_x_tau_x_error = np.matmul(
									np.exp(
										...
										error_mat
										).reshape(self.num_children, self.num_tables_child, num_symbols)
		if not self.log_varpar_assignment.size:
			self.log_varpar_label=np.zeros(self.log_varpar_label.shape)
		
	def _update_varpar_stick(self):
# 		print np.sum(self.log_varpar_assignment, axis=(0, 1))[:-1]
# 		print np.sum(self.log_varpar_assignment, axis=(0, 1))
		varpar_assignment=np.exp(self.log_varpar_assignment)
		self.varpar_stick[...,0] = np.sum(varpar_assignment, axis=(0, 1))[:-1]+1
		self.varpar_stick[...,1] = np.sum(
										np.cumsum(
											varpar_assignment[...,::-1],
											axis=-1
...
# 		self.varpar_stick[...,0] = np.sum(self.log_varpar_assignment, axis=0)[:-1]#+1
# 		self.varpar_stick[...,1] = np.sum(
# 										np.cumsum(
# 											self.log_varpar_assignment[:,::-1],
# 											axis=1
# 												)[:,::-1],
...
# 		self.log_varpar_assignment = self.log_varpar_assignment.reshape(self.num_children, self.num_tables_child, self.T)
		...
		self.log_varpar_assignment=(
									log_varpar_assignment
									).reshape(self.num_children, self.num_tables_child, self.T)
		self.phi_x_tau_x_error = np.matmul(
									np.exp(
										lla.logmatmul(
											log_varpar_assignment,
...
# 									)
								...
								+np.matmul(
									np.exp(
										lla.logmatmul(
											np.transpose(self.log_varpar_assignment.reshape(
																		self.num_children*self.num_tables_child,
																		self.T
																		)
...
# 		self.log_varpar_label /= denom+(denom==0).astype(np.float64)
	...
	def update_varpars(self):
		if self.log_varpar_assignment.size:
			children_label_np = np.array([child.log_varpar_label for child in self.children]).reshape(
																	self.num_children*self.num_tables_child,
																	num_symbols
																	)# (num_children*num_tables_child)x|∑|
			self._update_varpar_stick()
			assert not np.any(np.isnan(self.varpar_stick)), ('stick\n',self.varpar_stick)
			self._update_log_varpar_label(children_label_np)
			assert not np.any(np.isnan(self.log_varpar_label)), ('label\n',self.log_varpar_label)
			self._update_log_varpar_assignment(children_label_np)
			assert not np.any(np.isnan(self.log_varpar_assignment)), ('assignment\n',self.log_varpar_assignment)
# 			self.tau_x_error = np.matmul(
# 									self.log_varpar_label,
# 									error_mat
...
# 			print 'cumsum_phis', cumsum_phis 
	...
	# 					+
					...
					+np.sum(
# 							np.matmul(
# 								self.mother.log_varpar_assignment[self.id],
# 								self.mother.tau_x_error
# 								)
							self.mother.phi_x_tau_x_error[self.id]
							*np.exp(self.log_varpar_label)
						) # E[log p(eta_m | Z_m-1, eta_m-1)]
					+np.sum(
						digamma_diffs[:,1]*np.sum(cumsum_phis[...,1:], axis=(0,1))
						+
						digamma_diffs[:,0]*np.sum(varpar_assignment[...,:-1], axis=(0,1))
						) # E[log p(Z | V)]
	# 				+np.sum(
...
# 			print 'intermediate'
# 			print 'sum_E_log_varpar_stick',sum_E_log_varpar_stick
			sum_E_log_varpar_label = np.sum(self.log_varpar_label*np.exp(self.log_varpar_label))
# 			print 'sum_E_log_varpar_label', sum_E_log_varpar_label
			sum_E_log_varpar_assignment = np.sum(self.log_varpar_assignment*np.exp(self.log_varpar_assignment))
# 			print 'sum_E_log_varpar_assignment', sum_E_log_varpar_assignment, self.log_varpar_assignment
...
# 		num_symbols = num_symbols
		self.varpar_stick = np.random.gamma(10, 0.1, size=(T-1,2)) # gamma in Blei and Jordan.
		self.log_varpar_label = np.log(np.random.dirichlet(np.ones(num_symbols), T)) # Emission probability approximated by a multinomial distribution.
		self.concent = concent
		self.T = T
		self.new_id = 0
		self.customers=[]
	...
	def set_log_varpar_assignment(self):
		self.log_varpar_assignment = np.log(np.random.dirichlet(np.ones(self.T), len(self.customers))) # phi in Blei and Jordan.
		self.phi_x_tau_x_error = np.matmul(
										np.exp(
											lla.logmatmul(
												self.log_varpar_assignment,
	...
	def _update_varpar_stick(self):
# 		print 'log_varpar_assignment',self.log_varpar_assignment
# 		print np.sum(self.log_varpar_assignment, axis=0)[:-1s]
		varpar_assignment=np.exp(self.log_varpar_assignment)
		self.varpar_stick[...,0] = np.sum(varpar_assignment, axis=0)[:-1]+1
		self.varpar_stick[...,1] = np.sum(
										np.cumsum(
	...
	def _update_log_varpar_assignment(self):
		...
		log_varpar_assignment = (
									...
									+np.matmul(
										...
										np.transpose(np.exp(self.log_varpar_label)) # |∑|xT
										)
								)
# 		self.log_varpar_assignment /= np.sum(self.log_varpar_assignment, axis=-1)[:,np.newaxis]
		self.log_varpar_assignment=(
									log_varpar_assignment-spm.logsumexp(log_varpar_assignment, axis=-1)[:,np.newaxis]
									)
	...
	def _update_log_varpar_label(self):
# 		print 'update label'
# 		print np.exp(
# 								np.matmul(
# 									self.mother.log_varpar_assignment[self.id], # Txmother_T
...
# 									)
# 								+np.matmul(
# 									np.transpose(self.log_varpar_assignment), # TxN
# 									error_mat[self.customers] # Nx|∑|
# 									)
# 								)
# 		print self.log_varpar_assignment
...
# 									)
								...
								+
								np.matmul(
									np.transpose(np.exp(self.log_varpar_assignment)), # TxN
									self.hdp.get_word_assignment_weights(self.word_ids)[:,np.newaxis]
									*
									error_mat[self.customers] # Nx|∑|
									) #Tx|∑|
								)
# 		self.log_varpar_label /= np.sum(self.log_varpar_label, axis=-1)[:, np.newaxis]
		self.log_varpar_label=(log_varpar_label-spm.logsumexp(log_varpar_label, axis=-1)[:, np.newaxis])
...
# 						+
# 						digamma_diffs[:,0]*np.sum(self.log_varpar_assignment[:,:-1], axis=0)
# 						)
			output= (
					np.sum(
	# 					digamma_diffs[:,0]
...
# 								)
							self.mother.phi_x_tau_x_error[self.id]
							*np.exp(self.log_varpar_label) # Tx|∑|
						) # E[log p(eta_m | Z_m-1, eta_m-1)]
					+np.sum(
						digamma_diffs[:,1]*np.sum(cumsum_phis[:,1:], axis=0)
						+
						digamma_diffs[:,0]*np.sum(varpar_assignment[:,:-1], axis=0)
						) # E[log p(Z | V)]
	# 				+np.sum(
	# 					digamma_diffs[:,0]*self.varpar_stick[:,0]
	# 					+
...
# 			assert sum_E_log_varpar_stick<=0, (sum_E_log_varpar_stick,self.varpar_stick) #pdfだから、logが0以下とは限らない。
			sum_E_log_varpar_label = np.sum(self.log_varpar_label*np.exp(self.log_varpar_label))
			assert sum_E_log_varpar_label<=0, (sum_E_log_varpar_label,self.log_varpar_label)
			sum_E_log_varpar_assignment = np.sum(self.log_varpar_assignment*np.exp(self.log_varpar_assignment))
...
# 					digamma_diffs[:,1]*np.sum(cumsum_phis[...,:-1], axis=(0,1))
# 					+
# 					digamma_diffs[:,0]*np.sum(self.log_varpar_assignment[...,:-1], axis=(0,1))
# 					)
		output= (
				np.sum(
# 					digamma_diffs[:,0]
...
# 				-np.log(num_symbols)*self.log_varpar_label.shape[0] # E[log p(eta_1)]=-log(|∑|)<-constant.
				+np.sum(
					digamma_diffs[:,1]*np.sum(cumsum_phis[...,1:], axis=(0,1))
					+
					digamma_diffs[:,0]*np.sum(varpar_assignment[...,:-1], axis=(0,1))
...
def code_data(str_data):
	...
	coded_data = [[coder[s] for s in phrase] for phrase in str_data]
# 	print coded_data
