...
class VariationalInference(object):
	...
	def save_results(self, decoder):
		...
		with pd.HDFStore(
			...
			# Forward ngram
			for context_length,level in self.hdp_ngram.forward_tree.items():
...
# 						encoding="utf-8"
			...
			# Backward ngram
			for context_length,level in self.hdp_ngram.backward_tree.items():
...
class HDPNgram(object):
	def __init__(
		...
		self.forward_tree = {k:{} for k in range(n)}
		self.backward_tree = {k:{} for k in range(n)}
	...
	def set_varpar_assignment(self):
		...
		[restaurant.set_varpar_assignment()
			for level in self.forward_tree.values()
				for restaurant in level.values()
				]
		[restaurant.set_varpar_assignment()
			for level in self.backward_tree.values()
				for restaurant in level.values()
				]
	...
	def get_var_bound(self):
		return (
				np.sum([restaurant.get_var_bound()
					for level in reversed(list(self.forward_tree.values()))
						for restaurant in level.values()
						])
				+
				np.sum([restaurant.get_var_bound()
					for level in reversed(list(self.backward_tree.values()))
						for restaurant in level.values()
						])
				+
