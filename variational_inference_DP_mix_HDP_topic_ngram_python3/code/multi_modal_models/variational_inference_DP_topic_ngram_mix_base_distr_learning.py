...
class HDPNgram(object):
	def __init__(self, num_clusters, n, concent_priors, dirichlet_base_counts, full_contexts, num_sublex, backoff_direction_priors, customers, wrapper):
		...
		self.n = n
		self.tree_shared = {k:{} for k in range(n)}
		self.tree = {k:{} for k in range(n)}
		self.varpar_concents_shared = [VarParConcent(concent_priors) for context_length in range(n)]
	...
	def set_varpar_assignment(self):
		[restaurant.set_varpar_assignment()
			for level in self.tree_shared.values()
	...
	def get_var_bound(self):
		return (
				...
				+
				np.sum([restaurant.get_var_bound()
						for level in reversed(list(self.tree_shared.values()))
						for restaurant in level.values()
						])
				+
