...
def get_ngram_frequency(mat_assignments, data, start_symbol, end_symbol, n, df_ngram):
	df_ngram = df_ngram.sort_values('sublex_id')
	df_ngram['frequency'] = 0
	df_ngram['expected_frequency'] = np.float64(0)
	for string,sublex_assignment in zip(data, mat_assignments):
		# print ''.join(string)
		for ngram_window in zip(*[([start_symbol]*(n-1)+string+[end_symbol])[i:] for i in range(n)]):
			context = '_'.join(ngram_window[:-1])
			value = ngram_window[-1]
			assert not context in ['9ː_9ː', '9ː_b', '9ː_k', '9ː_nː'], ''.join(string)
			df_ngram.loc[
...
if __name__ == '__main__':
	...
	df_ngram['decoded_context'] = df_ngram.context.map(lambda context: '_'.join([decoder[int(code)] for code in context.split('_')]))
	...
	# data = [string.split(',') for string in df_data.DISC_csv.tolist()]
	...
	mat_assignments = df_sublex_assignment.rename(
							columns={
									...
									).ix[:,list(range(num_sublex))].values
	df_ngram = get_ngram_frequency(mat_assignments, data, 'START', 'END', n, df_ngram)
	...
	df_ngram.to_csv(
		os.path.join(
			result_dir,
			'posterior_%igram_with-frequency_with-end-symbol.csv' % (n)
			)
			,
			encoding = 'utf-8',
			index = False
