...
def get_3dbar(df, xcol, ycol, zcol, bottom = 0):
	...
	ax1.set_xticks(np.array([pos for value,pos in list(x2pos.items())]) + 0.5*width)
	ax1.set_xticklabels([value for value,pos in list(x2pos.items())])
	ax1.set_yticks(np.array([pos for value,pos in list(y2pos.items())]) + 0.5*width)
	ax1.set_yticklabels([value for value,pos in list(y2pos.items())])
...
if __name__ == '__main__':
	...
	df.loc[:,'prefix'] = df.prefix.str.replace('r','ɾ').str.replace('ä', r'a')
