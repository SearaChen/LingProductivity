...
# 			noise,
		...
		self.logger.info('DP mixture of words. Script last updated at %s'
							% datetime.datetime.fromtimestamp(
									os.stat(sys.argv[0]).st_mtime
									).strftime('%Y-%m-%d-%H:%M:%S')
							)
		self.logger.info('The base distribution is %i-gram with DP backoff.' % n)
		self.logger.info('%i-gram with DP backoff.' % n)
...
# 		print np.exp(error_mat).sum(axis=1)
		...
		self.logger.info('Gamma priors on concent_priorsration: (%f,%f)'
...
# 		return self.hdp_ngrams.get_var_bound()
		...
		return (
				(
						...
						-
						self.concent_mean*self.varpar_concent[1]
						+
...
class DP(object):
	def __init__(self, T, concent_priors, mother):
		self.mother = mother
		self.concent_priors = concent_priors
		self.varpar_concent=np.zeros(2)
		self.varpar_concent[0] = self.concent_priors[0]+T-1 # The shape of a gamma distr.
...
class DP_bottom(object):
	def __init__(self, T, concent_priors, mother, hdp):
		self.hdp=hdp
		self.mother = mother
		self.concent_priors = concent_priors
		self.varpar_concent=np.zeros(2)
...
class DP_top(object):
	def __init__(self, T, concent_priors):
		self.concent_priors = concent_priors
		self.varpar_concent=np.zeros(2)
		self.varpar_concent[0] = self.concent_priors[0]+T-1 # The shape of a gamma distr.
		self.varpar_concent[1] = np.random.gamma(self.concent_priors[1]**-1) # The INVERSE scale of a gamma distr.
