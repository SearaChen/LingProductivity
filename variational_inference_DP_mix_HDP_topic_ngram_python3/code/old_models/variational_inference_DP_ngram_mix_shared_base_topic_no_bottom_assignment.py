...
# 						encoding="utf-8"
			...
			df_concent['context_length']=list(self.hdp_ngram.tree.keys())
...
# 							encoding='utf-8'
			...
			for sblx_id,sublex in enumerate(self.sublex_ngrams):
				for context,bottom_rst in sublex.restaurants.items():
...
# 						encoding='utf-8'
	...
	def _update_varpar_concent(self):
		self.varpar_concent[1] = self.concent_priors[1]-np.sum(self.E_log_stick[:,1])
		self.concent_mean=self.varpar_concent[0]/self.varpar_concent[1]
	...
	def get_var_bound(self):
		...
		return (
				(
						(self.concent_mean-1)*np.sum(self.E_log_stick[:,1]) # E[alpha-1]*E[log (1-V)]
						-
				...
				+(
					np.sum(self.E_log_stick[:,1]*np.sum(np.cumsum(
															self.varpar_assignment[...,:0:-1],
															axis=-1
															)[...,::-1], axis=0))
					+
					np.sum(self.E_log_stick[:,0]*np.sum(self.varpar_assignment[:,:-1], axis=0))
					) # E[log p(Z | V)]
				+np.sum(
					self.log_like # num_words x num_sublex
					*
					self.varpar_assignment
					) # E[log p(X | Z,eta)]
				-
...
class DP_bottom(object):
	...
	def _update_varpar_stick(self):
		...
		self.varpar_stick[...,1] = np.sum(
										...
										axis=1)+self.sublex.concent_mean
		self.sum_stick = np.sum(self.varpar_stick, axis=-1)
		self.E_log_stick = sps.digamma(self.varpar_stick)-sps.digamma(self.sum_stick)[:, np.newaxis]
	...
	def _update_log_assignment(self):
		self.log_assignment=(
									...
									+
									self.mother.log_like_top_down[self.id] # num_clusters x num_symbols
								)

	def _update_log_like(self):
		self.log_like = spm.logsumexp(self.log_assignment, axis=0)
	...
	def _update_expected_customer_counts(self):
		expected_customers_per_symbol = np.zeros(num_symbols)
		np.add.at(
				expected_customers_per_symbol,
				self.customers,
				self.sublex.get_word_assignment_weights(self.word_ids)
				)
		self.expected_customer_counts=( # num_clusters x num_symbols
				expected_customers_per_symbol[np.newaxis,:]
				*
	...
	def _get_assign_probs(self):
		return np.exp(
					self.log_assignment # num_clusters x num_symbols
					-
					spm.logsumexp(self.log_assignment, axis=0)[np.newaxis,:]
					)

	def _update_log_like_bottom_up(self):
		self.log_like_bottom_up=self.expected_customer_counts # num_clusters x num_symbols
	...
	def get_sum_E_log_p_varpars(self):
		return (
					(
						(self.sublex.concent_mean-1)*np.sum(self.E_log_stick[:,1]) # E[alpha-1]*E[log (1-V)]
...
class DP(DP_bottom):
	...
	def get_sum_E_log_p_varpars(self):
		return (
					(
						(self.hdp.concent_mean[self.context_length]-1)*np.sum(self.E_log_stick[:,1]) # E[alpha-1]*E[log (1-V)]
					) # E[log p(V, alpha)] joint distr is easier to compute than likelihood and prior independently.
					+
					np.sum(
